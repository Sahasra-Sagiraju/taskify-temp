...
const updateUrgentTasksList = () => {
  ...
  tasksArrayCopy.forEach((task, index) => {
    urgentTasksContainer.insertAdjacentHTML(
      "beforeend",
      `<div class="urgent-tasks-details task-${index + 1}">
...
const renderDetailsCard = (sNo, task) => {
  ...
  backBtn.addEventListener("click", () => {
    ...
    urgentTasksContainer.removeAttribute("hidden");
  });
...
urgentTasksContainer.addEventListener("click", (event) => {
  if (event.target.classList.contains("details-btn")) {
    urgentTasksContainer.setAttribute("hidden", true);
